...
return new class extends Migration
{
    ...
    {
        $procedure = "DROP PROCEDURE IF EXISTS ProductUpload;
            ...
            insert into products (id, stone_id, cert_no, cert_type, cert_url, image, video, diamond_type,
                ...
                availability, treatment, inscription, comment
            )
            ...
            tp.rapo_rate, (tp.carat * tp.rapo_rate) as rapo_amount, tp.discount, tp.rate, tp.amount, tp.table, tp.table_per, tp.depth, tp.depth_per, tp.measurement, tp.length, tp.width, tp.height, tp.ratio,
            ...
            tp.availability, tp.treatment, tp.inscription, tp.comment

            from
            temp_products tp
            left join products p1 ON tp.stone_id=p1.stone_id
            left join products p2 ON tp.stone_id=p2.cert_no
            where tp.uuid=P_uuid and p1.stone_id is null and p2.cert_no is null and tp.imported_at is null and tp.import_type=1
            GROUP by tp.stone_id
            order by tp.id;

            update temp_products
            set imported_at=now()
            where uuid=P_uuid AND imported_at is null and import_type=1;
            END;";
    ...
    {
        // Schema::dropIfExists('product_uploadprocedure');
    }
};
